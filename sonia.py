...
#Import OLGA for seq generation
...
#    data= np.sum((-y_pred)*(1.-y_true))/np.sum(1.-y_true)
#    gen= np.log(np.sum(np.exp(-y_pred)*y_true))-np.log(np.sum(y_true))
#    reg= np.exp(gen)-1.
...
class computeL1(keras.callbacks.Callback):
    """Callback class to allow for tracking of marginals L1 convergence"""
    ...
    def return_model_marginals(self):
        ...
        Qs = np.exp(-self.model.predict(self.sonia._encode_data(gen_enc))[:, 0])
