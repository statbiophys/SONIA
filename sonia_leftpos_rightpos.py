...
class SoniaLeftposRightpos(Sonia):
	...
	def __init__(self, data_seqs = [], gen_seqs = [], chain_type = 'humanTRB', load_model = None, max_depth = 25, max_L = 30, include_genes = True,sample_gen=False,size_sample=int(5e4)):
				
		Sonia.__init__(self, data_seqs=data_seqs, gen_seqs=gen_seqs, chain_type=chain_type, load_model = load_model,sample_gen=sample_gen,size_sample=size_sample)
	...
	def add_features(self, include_genes = True):
		...
		if include_genes:
			...
			main_folder = os.path.join(os.path.dirname(olga_load_model.__file__), 'default_models', 'human_T_beta')
